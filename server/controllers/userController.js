...
export const registerUser = (pool) => {
  return async (req, res) => {
    const { username, email, password } = req.body;

    try {
      // Check if the user already exists by email or username
      const existingUser = await userModel.findUserByEmailOrUsername(pool, email, username);
      if (existingUser) {
      ...
      // Create a new user
      const newUser = await userModel.createUser(pool, { username, email, password: hashedPassword });
...
export const getAllUsers = (pool) => {
  return async (req, res) => {
    try {
      const users = await userModel.getAllUsers(pool); // Fetch all users using the model
      res.json(users);
    } catch (error) {
